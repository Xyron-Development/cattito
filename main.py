...
# Copyright (C) 2025 Lia F34R.mp4! And Noxx Studios & Cattito Contributors
#
...
hints = [
    ...
    "Cattito's birthday is on the 21st of April",
    ...
    "Cattito once tried to nap in 100 different virtual boxes at once",
    "Cat died for 2+ weeks bc the servers were flooded with water",
    "Did you know cats have over 20 muscles in their ears?",
    "A group of cats is called a clowder",
    "Cats sleep for 12-16 hours a day. Lucky them!",
    "Cattito has a secret cat dance move 🐾",
    "Cats can make over 100 different sounds",
    "Cattito sometimes hides in virtual cardboard boxes",
    "If you put a cat in a paper bag, it will probably sit there forever",
    "Cats have whiskers on the backs of their front legs too",
    "Cattito loves virtual catnip!",
...
funny = [
    ...
    "bro thought he had admin 💀",
    "you really tried that… bold move",
    "this ain’t your playground",
    "error 404: your permissions not found",
    "nice try, hacker man",
    "you pressing buttons like it'll change something",
    "who let you cook",
    "not in a million years",
    "you must be lost 👀",
    "I admire the confidence, not the decision",
    "even my grandma knows not to click that",
    "you’re about as authorized as a potato",
    "please return to the kiddie pool",
    "your click has been reported to the Ministry of Silly Attempts",
    "this action has been yeeted into the void",
    "stop before I call tech support",
    "your enthusiasm is unmatched… unfortunately, so is your lack of permission",
    "big no energy",
    "this is why we can’t have nice things",
    "I’d say yes, but I enjoy watching you fail",
]
...
# ???
rigged_users = [1075077561454973020]
...
MANUAL_OWNER_ID = 1075077561454973020
...
# news stuff
news_list = [
    {"title": "Chapter I: The Vanishing of Catropolis", "emoji": "🌀"},
    {"title": "Dev Log #14: Dimensional Anchors", "emoji": "🔮"},
    {"title": "Event: The Shattered Sky", "emoji": "🌌"},
    {"title": "Lore Drop: The First Cat", "emoji": "🧠"},
    {"title": "Upcoming Feature: Cat Expeditions", "emoji": "🚀"},
    {"title": "Developer Note", "emoji": "🐱‍👓"},
]
    ...
    if perms.send_messages and (not isinstance(message.channel, discord.Thread) or perms.send_messages_in_threads):
        ...
        await message.channel.send(
            ...
            .set_author(
                ...
                icon_url="https://wsrv.nl/?url=raw.githubusercontent.com/F34R.mp4! And Noxx Studios/cat-bot/main/images/cat.png",
            )
...
async def send_cheat_alert(user: discord.User, guild: discord.Guild, channel: discord.TextChannel, attempts: int):
    """Sends a detailed anti-cheat notification with an embed, link buttons, and pings a user outside the embed."""

    global anticheat_cooldown
    current_time = time.time()

    # Prevent spam alerts
    if user.id in anticheat_cooldown and current_time < anticheat_cooldown[user.id]:
        return

    anticheat_cooldown[user.id] = current_time + ANTISPAM_ALERT_COOLDOWN
    ...
    embed = {
        ...
        "fields": [
            ...
            {"name": "Reason", "value": "Spamming Catch Command", "inline": False},
            {"name": "Details", "value": f"Sent {attempts} 'cat' messages in under {ANTISPAM_WINDOW} seconds.", "inline": False}
        ],
        "footer": {"text": "Check this out!"},
    ...
    buttons = [
        ...
        },
        {
            ...
            "style": 5,
            "label": "Join Server",
            "url": str(await guild.vanity_invite()) if hasattr(guild, "vanity_invite") else f"https://discord.gg/{guild.id}"
        }
    ]

    # Ping user outside the embed
    webhook_payload = {
        "content": f"<@{PING_USER_ID}>",  # This pings the user
        "username": "Cattito Anti-Cheat",
        ...
        try:
            async with session.post(WEBHOOK_URL, json=webhook_payload) as response:
                if response.status >= 400:
                    logging.error(f"Failed to send anti-cheat webhook: {response.status} - {await response.text()}")
        except Exception as e:
            logging.error(f"Error sending anti-cheat webhook: {e}")


async def check_for_spam(message: discord.Message) -> bool:
    """Checks for cat catch spamming using a sliding window.
    Returns True if user is cheating/spamming (and the catch should be denied), False otherwise."""

    # Only check 'cat' messages in a guild
    if message.content.lower() != "cat" or not message.guild:
        return False
        
    global catch_spam_tracker
    now = time.time()
    user_id = message.author.id
    
    if user_id not in catch_spam_tracker:
        catch_spam_tracker[user_id] = []

    # 1. Add the current attempt
    catch_spam_tracker[user_id].append((now, message.channel.id))

    # 2. Filter out attempts older than the window (sliding window logic)
    catch_spam_tracker[user_id] = [
        (t, c) for t, c in catch_spam_tracker[user_id] if now - t < ANTISPAM_WINDOW
    ]

    # 3. Check for cheating
    attempt_count = len(catch_spam_tracker[user_id])
    
    if attempt_count >= ANTISPAM_LIMIT:
        # Cheating detected! Send the alert and block the catch.
        await send_cheat_alert(message.author, message.guild, message.channel, attempt_count)
        
        # A good anti-cheat is often silent, so we just deny the catch and return True.
        return True
...
async def maintaince_loop():
    # In your maintaince_loop function:
    global pointlaugh_ratelimit, reactions_ratelimit, last_loop_time, loop_count, catchcooldown, fakecooldown, anticheat_cooldown, temp_belated_storage, temp_cookie_storage 
# Note: Ensure this list matches your existing globals, but includes anticheat_cooldown
    pointlaugh_ratelimit = {}
    reactions_ratelimit = {}
    catchcooldown = {}
    fakecooldown = {}
...
# Anti-cheat cooldown cleanup <-- ADD THIS BLOCK
    current_time = time.time()
    keys_to_delete = [k for k, v in anticheat_cooldown.items() if v < current_time]
    for k in keys_to_delete:
        del anticheat_cooldown[k]
...
async def on_ready():
    ...
    # Force OWNER_ID to your manual ID
    OWNER_ID = MANUAL_OWNER_ID

    # Optional: print to verify
    print(f"Bot owner is set to: {OWNER_ID}")
...
1075077561454973020,
    ...
    url = "https://api.github.com/repos/F34R.mp4! And Noxx Studios/cat-bot/contributors"
    contributors = []

    async with aiohttp.ClientSession() as session:
        async with session.get(url, headers={"User-Agent": "CatBot/1.0 https://github.com/F34R.mp4! And Noxx Studios/cat-bot"}) as response:
            if response.status == 200:
                data = await response.json()
                for contributor in data:
                    login = contributor["login"].replace("_", r"\_")
                    if login not in ["F34R.mp4! And Noxx Studios", "ImgBotApp"]:
...
async def on_message(message: discord.Message):
    ...
    # If check_for_spam returns True, the user is cheating/spamming,
    # and the function returns immediately, blocking the catch.
    if await check_for_spam(message):
        return
    # ----------------------------
    ...
    achs = [
        ...
        # New triggered phrases
        ["hello cat", "exact", "hello_cat"],
        ["i love cats", "exact", "i_love_cats"],
        ["lag", "exact", "lag_king"],
        ["bruh", "exact", "bruh"],
        ["meow", "exact", "meow"],
        ["oops", "exact", "oops"],
        ["pizza time", "exact", "pizza_time"],
        ["troll cat", "exact", "troll_cat"],
        ["big brain", "exact", "big_brain"],
        ["cat in a cat in a cat", "exact", "catception_phrase"],
        ["hello world", "exact", "hello_world"],
        ["why", "exact", "why"],
        ["stupid cat", "exact", "stupid_cat"],
        ["amogus", "exact", "amogus"],
        ["yikes", "exact", "yikes"],
        ["f34r", "exact", "f34r"],
        ["F34R", "exact", "f34r"],
    ...
    # NOTE: Ensure you call await bot.process_commands(message) at the end of your full function,
    # if it's not already included in your full script.
    ...
    if config.RAIN_CHANNEL_ID and message.channel.id == config.RAIN_CHANNEL_ID and text.lower().startswith("cat!rain"):
        ...
        try:
            ...
            await person.send(
                f"**You have recieved {things[2]} minutes of Cat Rain!** ☔\n\nThanks for your support!\nYou can start a rain with `/rain`. By buying you also get access to `/editprofile` command as well as a role in [our Discord server](<https://discord.gg/GCYRuTWnpr>), where you can also get a decorative custom cat!\n\nEnjoy your goods!"
            )
        except Exception:
            pass
...
# Apply multiplier only if machine is active
                    current_time = int(time.time())
                    if getattr(user, "cataine_active", 0) > current_time:
                        boost_mult = getattr(user, "cataine_multiplier", 2) or 2
                        silly_amount *= boost_mult
                        suffix_string += f"\n🧂 Cataine worked! Your catch got multiplied by {boost_mult}x!"
                        user.cataine_activations += 1
                    else:
                        boost_mult = 1  # expired, no boost
                ...
                custom_cough_strings = {
                    ...
                    "eboy": "{username} cawght {emoji} {type} cat... 😎✨\nYou now vibe with {count} cats of dat type 🖤\nthis lil' dude got snagged in {time} fr 😤🔥",
                    "unknown": "{username}... stumbled upon {emoji} {type} cat? ❓\n{count} cats now whisper your name in the dark 🕯️\nno one knows when this entity was caught... only {time} remains.",
                    "f34rmp4": "{username} just encountered the ✨ {emoji} {type} cat ✨\nA LEGENDARY soul tied to the bot's creator 👑\nYou now hold {count} of this mythical type...\nThis rare being was caught in {time} ⚡",
                    "Emo": "{username} cawght {emoji} {type} cat... 🖤\n{count} cats now linger in the shadows with you...\nthis lonely soul was taken in {time} 🌧️",
                    ...
                    "snacc": "🐾 {username} pounced on a {emoji} {type} !\nNom nom nom... the kitty is munching happily~ 🍽️\nYou now have {count} tasty cats of that type!!\nThis lil’ snacko was caught in {time}! 🕒"
                }
                ...
                if random.randint(0, 10) == 0 and user.cat_Fine >= 20 and not user.dark_market_active:
                    ...
                    button = Button(label="Join our Discord!", url="https://discord.gg/GCYRuTWnpr")
                elif random.randint(0, 500) == 0:
                    button = Button(label="John Discord 🤠", url="https://discord.gg/GCYRuTWnpr")
                elif random.randint(0, 50000) == 0:
                    button = Button(
                        label="DAVE DISCORD 😀💀⚠️🥺",
                        url="https://discord.gg/GCYRuTWnpr",
                    )
    ...
    if text.lower().startswith("cat!rain"):
            ...
            # Extract only digits from the second argument
            user_id_str = re.sub(r'\D', '', stuff[1])
            if not user_id_str:
            ...
            # Initialize rain_minutes if needed
            if not hasattr(user, "rain_minutes") or user.rain_minutes is None:
                user.rain_minutes = 0

            # Handle rain duration
            rain_input = stuff[2] if len(stuff) > 2 else None
            if rain_input == "short":
                user.rain_minutes += 2
            elif rain_input == "medium":
                user.rain_minutes += 10
            elif rain_input == "long":
                user.rain_minutes += 20
            else:
                try:
                    user.rain_minutes += int(rain_input)
    ...
    if text.lower().startswith("cat!resetcataine"):
        stuff = text.split()

        if len(stuff) < 2:
            await message.reply("❌ Usage: `cat!resetcataine <user_id>`")
            return

        # Extract only digits from the second argument (in case of <@mentions>)
        user_id_str = re.sub(r'\D', '', stuff[1])
        if not user_id_str:
        ...
        try:
            ...
            profile.cataine_active = 0
            profile.cataine_week = 0
            await profile.save()

            await message.reply(f"✅ Successfully reset Cataine for <@{user_id}>.")
        except Exception as e:
            await message.reply(f"❌ Failed to reset Cataine: `{e}`")
...
# Only proceed if the message starts with "!cat"
    if not message.content.startswith("!cat"):
        return  # 👈 stops non-command messages from triggering the block
    ...
    if len(stuff) > 1:
        # Check if stuff[1] doesn't start with a digit → insert guild owner ID if available
        if stuff[1][0] not in "1234567890":
            owner_id = message.guild.owner_id if message.guild else None
            stuff.insert(1, str(owner_id) if owner_id else "None")

        # Clean stuff[1] to extract digits for user_id
        user_id_str = re.sub(r'\D', '', str(stuff[1]))
        if user_id_str:
            user_id = int(user_id_str)
            user = await User.get_or_create(user_id=user_id)

    # Build cat name safely
    cat_name = " ".join(stuff[2:]) if len(stuff) > 2 else None

    # Fetch emojis list once
    emojis = {emoji.name: str(emoji) for emoji in await bot.fetch_application_emojis()}

    # Only proceed with emoji creation if we have a valid cat_name and a reply with an image
    if cat_name and cat_name != "None" and message.reference and message.reference.message_id:
        ...
        if not og_msg or not og_msg.attachments:
    ...
    if user:
        user.custom = cat_name if cat_name else ""
        await user.save()
...
async def on_guild_join(guild):
    ...
    try:
        if ch.permissions_for(guild.me).send_messages:
            await ch.send(
                unofficial_note
                + "Thanks for adding me!\nTo start, use `/setup` and `/help` to learn more!\nJoin the support server here: https://discord.gg/GCYRuTWnpr\nHave a nice day :)"
            )
    except Exception:
        pass
...
async def help(message):
    ...
    embed2 = (
        ...
        .add_field(
            ...
            value='Cattito has various other mechanics to make fun funnier. You can collect various `/achievements`, for example saying "i read help", progress in the `/battlepass`, or have beef with the mafia over cataine addiction. The amount you worship is the limit!',
            inline=False,
        )
        .add_field(
            name="Other features",
            value="Cattito has extra fun commands which you will discover along the way.\nAnything unclear? Check out [our wiki](https://cattitio.fun) or drop us a line at our [Discord server](https://discord.gg/GCYRuTWnpr).",
            inline=False,
        )
        .set_footer(
            text=f"Cattito by F34R.mp4! And Noxx Studios, {datetime.datetime.utcnow().year}"
        )
    )
...
async def credits(message: discord.Interaction):
    ...
    embedVar = discord.Embed(title="Cattito", color=Colors.brown, description=gen_credits).set_thumbnail(
        url="https://wsrv.nl/?url=raw.githubusercontent.com/F34R.mp4! And Noxx Studios/cat-bot/main/images/cat.png"
    )
...
@bot.tree.command(description="📢 Read the Cattito Chronicle")
async def news(message: discord.Interaction):
    ...
    async def send_news(interaction: discord.Interaction):
        ...
        if news_id == 0:
            embed = Container(
                "## 🌀 Chapter I: The Vanishing of Catropolis",
                "Citizens awoke to find the entire western district of Catropolis gone. Streets end abruptly, buildings sliced in half as if erased. Authorities blame a rare ‘Catclipse’ anomaly — a mysterious storm that erases matter.\n\n🐾 Rumors speak of glowing puzzle fragments scattered across the realm...",
                "-# <t:1731168230>"
            )
            view.add_item(embed)
            ...
            embed = Container(
                "## 🔮 Dev Log #14: Dimensional Anchors",
                "Hey everyone! We're working on a new feature called **Dimensional Anchors**, which lets you stabilize Catclipse zones. Collect fragments through daily quests and anchor them to earn **Lore XP** and rare cat relics.\n\nAnchors will go live in v5.0 — stay tuned.",
                "-# <t:1732377932>"
            )
            view.add_item(embed)
            view.add_item(back_row)
            ...
            embed = Container(
                "## 🌌 Event: The Shattered Sky",
                "For 5 days only, mysterious cracks are appearing in the sky. Interact with them using `/investigate` to uncover lore pieces, rare celestial cats, and even unique sky runes!\n\n- 🪄 Each server gets different lore outcomes\n- 🌠 Unlock special profile frames by participating\n\nEvent runs until October 25th.",
                "-# <t:1734458962>"
            )
            view.add_item(embed)
            view.add_item(back_row)
            ...
            embed = Container(
                "## 🧠 Lore Drop: The First Cat",
                "According to ancient scrolls found in the Archive District, the **First Cat** predates recorded bot history. Its whiskers were said to weave the fabric of dimensions.\n\nNo one knows where it is now, but whispers suggest it still roams between servers, searching...",
                "-# <t:1735689601>"
            )
            view.add_item(embed)
            view.add_item(back_row)
            ...
            embed = Container(
                "## 🚀 Upcoming Feature: Cat Expeditions",
                "Soon you'll be able to send your cats on **expeditions** beyond your server. Expeditions run in real time and return with relics, fragments, XP, or sometimes… nothing.\n\nSome cats have special expedition perks 👀",
                "-# <t:1740787200>"
            )
            view.add_item(embed)
            view.add_item(back_row)
            ...
            embed = Container(
                "## 🐱‍👓 Developer Note",
                "We’ve been experimenting with new ways to make the world of Cattito feel more alive — dynamic stories, seasonal events, and permanent lore progression. Your feedback during the last test event was incredible.\n\nThank you for sticking with us — big things are coming.",
                "-# <t:1743454803>"
            )
            view.add_item(embed)
            view.add_item(back_row)
        ...
        # You can continue for more "news_id" entries with totally different stories or features...
    ...
    def generate_page(number):
        view = LayoutView(timeout=VIEW_TIMEOUT)
        view.add_item(TextDisplay("Choose a Chronicle Entry:"))
    ...
    async with aiohttp.ClientSession() as session:
        try:
            async with session.post(
                ...
                headers={"User-Agent": "CatBot/1.0 https://github.com/F34R.mp4! And Noxx Studios/cat-bot"},
            ) as response:
...
async def gen_inventory(message, person_id):
    ...
    escaped_name = person_id.name.replace("_", "\\_")  # do this before the f-string

    embedVar = discord.Embed(
    title=f"{emoji_prefix}{escaped_name}",
    description=(
        f"{highlighted_stat[1]} {highlighted_stat[2]}\n"
        f"{get_emoji('ach')} Achievements: {unlocked}/{total_achs}{minus_achs}\n"
        f"⬆️ Battlepass Level {person.battlepass} ({person.progress}/{needed_xp} XP)"
    ),
    ...
    async with aiohttp.ClientSession() as session:
        try:
            async with session.get(
                "https://api.thecatapi.com/v1/images/search", headers={"User-Agent": "CatBot/1.0 https://github.com/F34R.mp4! And Noxx Studios/cat-bot"}
            ) as response:
                data = await response.json()
...
define_cooldowns = {}
DEFINE_COOLDOWN = 60  # 1 minute per user
...
async def define(message: discord.Interaction, word: str):
    ...
    # cooldown check
    if user_id in define_cooldowns and now - define_cooldowns[user_id] < DEFINE_COOLDOWN:
        remaining = int(DEFINE_COOLDOWN - (now - define_cooldowns[user_id]))
        await message.response.send_message(
            f"⏳ Please wait {remaining}s before using this command again.", ephemeral=True
        )
        return
    ...
    async with aiohttp.ClientSession() as session:
        try:
            async with session.get(
                f"https://od-api.oxforddictionaries.com/api/v2/entries/en-us/{word}",
                headers={
                ...
                data = await response.json()
                text = (await response.text()).lower()  # for filtering offensive words

                # Oxford stores definitions nested in lexicalEntries -> entries -> senses
                        ...
                        for entry in lexical.get("entries", []):
                            for sense in entry.get("senses", []):
                                if "definitions" in sense:
                                    clean_data = re.sub(re.compile("<.*?>"), "", sense["definitions"][0])
                                    await message.response.send_message(
                                        f"__{word}__\n{clean_data}\n- Powered by [Oxford Dictionaries](https://www.oxfordlearnersdictionaries.com/)",
...
# ✅ Updated DURATION_OPTIONS with higher multipliers for 2x–10x boosts
DURATION_OPTIONS = {
    "12h": 1,    # base
    "24h": 2,
    "48h": 4,
    "2x": 8,    # expensive 2x boost
    "4x": 16,   # even more expensive
    "6x": 32,
    "8x": 64,
    "10x": 128, # ultra expensive
}  # multiplier for price and duration in seconds


# ✅ Updated DURATION_OPTIONS — 2x removed
# ✅ Separate price/duration multiplier and boost multiplier
DEV_USER_ID = 1075077561454973020
LAST_PRICE_DATE = None
DAILY_LIGHT_DEALS = []

DURATION_OPTIONS = {
    "12h": {"multiplier": 1, "boost": 2},
    "24h": {"multiplier": 2, "boost": 4},
    "48h": {"multiplier": 4, "boost": 6},
    "4x":  {"multiplier": 16, "boost": 4},
    "6x":  {"multiplier": 32, "boost": 6},
    "8x":  {"multiplier": 64, "boost": 8},
    "10x": {"multiplier": 128, "boost": 10},
}


async def light_market(message):
    global LAST_PRICE_DATE, DAILY_LIGHT_DEALS
    cataine_prices = [
        [10, "Fine"], [30, "Fine"], [20, "Good"], [15, "Rare"],
        [20, "Wild"], [10, "Epic"], [20, "Sus"], [15, "Rickroll"],
        [7, "Superior"], [5, "Legendary"], [3, "8bit"], [4, "Divine"],
        [3, "Real"], [2, "Ultimate"], [1, "eGirl"],
    ]
    ...
    if user.cataine_active < current_time:
        if user.recent_week != datetime.datetime.utcnow().isocalendar()[1]:
            user.cataine_week = 0
        ...
        if LAST_PRICE_DATE != today_str:
            ...
            DAILY_LIGHT_DEALS = random.sample(range(len(cataine_prices)), k=random.randint(3, 5))
            DAILY_LIGHT_DEALS = [cataine_prices[i] for i in DAILY_LIGHT_DEALS]
            random.setstate(state)
            LAST_PRICE_DATE = today_str

        if user.cataine_week >= len(DAILY_LIGHT_DEALS):
        ...
        deal = DAILY_LIGHT_DEALS[user.cataine_week]
        type_ = deal[1]
        base_amount = deal[0]
        ...
        # Show all durations with correct daily price
        for duration, data in DURATION_OPTIONS.items():
            price = 0 if message.user.id == DEV_USER_ID else base_amount * data["multiplier"]
            embed.add_field(
                name=f"🧂 {duration} of Cataine",
                value=f"Price: {get_emoji(type_.lower()+'cat')} {price} {type_}",
            )

        async def make_cataine(interaction, duration="12h"):
            await user.refresh_from_db()
            data = DURATION_OPTIONS[duration]
            price = 0 if user.user_id == DEV_USER_ID else base_amount * data["multiplier"]

            if (user[f"cat_{type_}"] < price and user.user_id != DEV_USER_ID) or user.cataine_active > current_time:
                await interaction.response.send_message("Not enough cats or machine still active.", ephemeral=True)
                return

            # Deduct price only if not dev
            if user.user_id != DEV_USER_ID:
                user[f"cat_{type_}"] -= price

            # Store the actual boost and expiry timestamp
            user.cataine_multiplier = data["boost"]
            user.cataine_active = current_time + 43200 * data["multiplier"]  # 12h base * multiplier
            user.cataine_week += 1
            user.cataine_bought += 1
            await user.save()

            await interaction.response.send_message(
                f"The machine spools down. Your cat catches will be multiplied by {data['boost']}x for the next {duration}.",
                ephemeral=True,
            )
        ...
        for duration, data in DURATION_OPTIONS.items():
            price = 0 if user.user_id == DEV_USER_ID else base_amount * data["multiplier"]
            if user[f"cat_{type_}"] >= price or user.user_id == DEV_USER_ID:
                button = Button(label=f"Buy {duration}", style=ButtonStyle.blurple)
                button.callback = lambda inter, dur=duration: make_cataine(inter, dur)
            else:
                button = Button(label=f"Not enough cats for {duration}", disabled=True)
            myview.add_item(button)
...
async def dark_market(message):
    cataine_prices = [
        [5, "Fine"], [5, "Good"], [4, "Wild"], [4, "Epic"],
        [3, "Brave"], [3, "Reverse"], [2, "Trash"], [2, "Mythic"],
        [1, "Corrupt"], [1, "Divine"], [100, "eGirl"],
    ]
    ...
    if user.cataine_active < current_time:
        ...
        embed.set_author(name="Click here to open Wiki", url="https://cattito.fun/dark")
        ...
        async def buy_cataine(interaction, duration="12h"):
            await user.refresh_from_db()
            data = DURATION_OPTIONS[duration]
            price = 0 if user.user_id == DEV_USER_ID else base_amount * data["multiplier"]
            ...
            # Store boost and expiry
            user.cataine_multiplier = data["boost"]
            user.cataine_active = current_time + 43200 * data["multiplier"]
            user.dark_market_level += 1
            user.cataine_bought += 1
            await user.save()

            await interaction.response.send_message(
                f"Thanks for buying! Your cat catches will be multiplied by {data['boost']}x for the next {duration}.",
                ephemeral=True,
            )
        ...
        for duration, data in DURATION_OPTIONS.items():
            ...
            if user[f"cat_{type_}"] >= price or user.user_id == DEV_USER_ID:
                button = Button(label=f"Buy {duration}", style=ButtonStyle.blurple)
                button.callback = lambda inter, dur=duration: buy_cataine(inter, dur)
            else:
                button = Button(label=f"Not enough cats for {duration}", disabled=True)
            myview.add_item(button)
        ...
        last_purchase_time = datetime.datetime.fromtimestamp(user.cataine_active)
        await message.followup.send(
            embed=discord.Embed(
                title="The Dark Market",
                description=f"you already bought from us recently. Your last purchase was at {last_purchase_time.strftime('%Y-%m-%d %H:%M:%S UTC')}."
            ),
            ephemeral=True
        )
...
cooldowns = {}
COOLDOWN_SECONDS = 30  # adjust as needed

# Replace Colors.brown with your actual Colors object if you have one
class Colors:
    brown = 0xA52A2A

# Slash command
@bot.tree.command(description="Generate an AI image from a prompt!")
async def dream(interaction: discord.Interaction, prompt: str):
    ...
    if user_id in cooldowns and now - cooldowns[user_id] < COOLDOWN_SECONDS:
        ...
        await interaction.response.send_message(f"⏳ Please wait {remaining}s before generating another image.", ephemeral=True)
        return
    ...
    if response.status_code == 200:
        ...
        # Create embed like your wiki command
        embed = discord.Embed(title="Dream Image", color=Colors.brown)
        embed.description = f"Prompt: {prompt}"
        embed.set_image(url="attachment://dream.png")

        await interaction.response.send_message(embed=embed, file=discord.File(img_data, "dream.png"))

        # Update cooldown
        cooldowns[user_id] = now

        # Get profile and run progress function
        profile = await Profile.get_or_create(guild_id=interaction.guild.id, user_id=interaction.user.id)
        await progress(interaction, profile, "dream")
    else:
        await interaction.response.send_message("❌ Failed to generate image. Try again later.", ephemeral=True)
...
async def wiki(message: discord.Interaction):
    ...
    embed.description = "\n".join(
        [
            ...
            "",
            "[Cattito](https://cattito.fun)",
            "[Cat Spawning](https://cattito.fun)",
            "[Commands](https://cattito.fun)",
            "[Cat Types](https://cattito.fun)",
            "[Cattlepass](https://cattito.fun)",
            "[Achievements](https://cattito.fun)",
            "[Packs](https://cattito.fun)",
            "[Trading](https://cattito.fun)",
            "[Gambling](https://cattito.fun)",
            "[The Dark Market](https://cattito.fun)",
            "[Prisms](https://cattito.fun)",
        ]
    )
    await message.response.send_message(embed=embed)
    profile = await Profile.get_or_create(guild_id=message.guild.id, user_id=message.user.id)
...
async def achievements(message: discord.Interaction):
    ...
    async def gen_new(category):
        ...
        total_achs = len(ach_list) - minus_achs_count
        minus_achs = f" + {minus_achs}" if minus_achs != 0 else ""
        ...
        global_user = await User.get_or_create(user_id=message.user.id)
        unread_news = len(news_list) > len(global_user.news_state.strip()) or "0" in global_user.news_state.strip()[-4:]
        ...
        embeds = []
        current_embed = make_base_embed()
        if unread_news:
            current_embed.set_author(name="You have unread news! /news")
        field_count = 0

        for f in fields_to_add:
            # Split when limits are reached
            if field_count >= 25 or len(str(current_embed.to_dict())) > 5900:
            ...
            current_embed.add_field(**f)
            field_count += 1

        if len(current_embed.fields) > 0 or not embeds:
            embeds.append(current_embed)
    ...
    def insane_view_generator(category):
        ...
        async def callback_hell(interaction):
            ...
            try:
                await interaction.edit_original_response(
                    embeds=await gen_new(thing),
                    view=insane_view_generator(thing)
                )
            except Exception:
                pass
        ...
        for num, i in enumerate(["Cat Hunt", "Commands", "Random", "Silly", "Hard", "Hidden"]):
            style = ButtonStyle.green if category == i else ButtonStyle.blurple
            button = Button(label=i, custom_id=i, style=style, row=num // 3)
            button.callback = callback_hell
...
# --- RESET SERVER CATS ---
@bot.tree.command(description="[DEV] Reset all cats in a specific server by ID")
async def resetservercats(message: discord.Interaction, server_id: str):
    if message.user.id != OWNER_ID:
        return await message.response.send_message("❌ You are not authorized to use this command.", ephemeral=True)

    guild = bot.get_guild(int(server_id))
    if not guild:
        return await message.response.send_message("⚠️ Server not found or bot not in that server.", ephemeral=True)

    embed = Embed(
        title="⚠️ Confirm Cat Reset",
        description=f"Are you **sure** you want to reset *everyone's cats* in **{guild.name}**?\nThis action **cannot be undone!**",
        color=Colors.demonic
    ).set_footer(text="Cattito Developer Command")
    ...
    async def confirm_callback(interaction: discord.Interaction):
        ...
        profiles = []
        async for user in Profile.limit(["id"], "guild_id = $1", int(server_id)):
            for cat_type in cattypes:
                setattr(user, f"cat_{cat_type}", 0)
            profiles.append(user)
        if profiles:
            await Profile.bulk_update(profiles, [f"cat_{cat}" for cat in cattypes])

        await message.followup.send(f"✅ All cats in **{guild.name}** ({guild.id}) have been reset.", ephemeral=True)

    async def cancel_callback(interaction: discord.Interaction):
        if interaction.user.id == message.user.id:
            await interaction.response.edit_message(content="❎ Cancelled cat reset.", view=None)

    confirm_button = Button(label="Confirm", style=ButtonStyle.danger, emoji="⚠️")
    ...
    await message.response.send_message(embed=embed, ephemeral=True, view=view)


# --- MODIFY USER CATS ---
@bot.tree.command(description="[DEV] Reset or give all cats to a user")
async def modifyusercats(message: discord.Interaction, server_id: str, user_id: str, give_all: bool):
    if message.user.id != OWNER_ID:
        return await message.response.send_message("❌ You are not authorized to use this command.", ephemeral=True)

    profile = await Profile.get_or_none(guild_id=int(server_id), user_id=int(user_id))
    if not profile:
        return await message.response.send_message("⚠️ User not found in that server.", ephemeral=True)

    for cat_type in cattypes:
        setattr(profile, f"cat_{cat_type}", 1 if give_all else 0)
    await profile.save()

    action = "gave 1 of each cat to" if give_all else "reset all cats for"
    embed = Embed(
        title="Cat Modification Complete",
        description=f"✅ Successfully {action} <@{user_id}> in `{server_id}`.",
        color=Colors.green
    ).set_footer(text="Cattito Developer Command")

    await message.response.send_message(embed=embed, ephemeral=True)
...
@bot.tree.command(name="servers", description="(Owner only) View all servers the bot is in")
async def servers(interaction: discord.Interaction):
    # Check ownership first
    if interaction.user.id != OWNER_ID:
        return await interaction.response.send_message("❌ You are not the bot owner.", ephemeral=True)

    # Defer the interaction to prevent timeout (ephemeral)
    await interaction.response.defer(ephemeral=True, thinking=True)

    guilds = bot.guilds
    if not guilds:
        return await interaction.followup.send("I'm not in any servers 🤔", ephemeral=True)

    embeds = []
    page_size = 10  # servers per page

    for i in range(0, len(guilds), page_size):
        chunk = guilds[i:i+page_size]
        embed = discord.Embed(
            title=f"📜 Bot Servers ({i+1}–{i+len(chunk)} of {len(guilds)})",
    ...
    class PageView(discord.ui.View):
        ...
        async def previous(self, button: discord.ui.Button, inter: discord.Interaction):
            nonlocal current
            current = (current - 1) % len(embeds)
            await inter.response.edit_message(embed=embeds[current], view=self)
    ...
    # Send the first embed using followup since we've deferred
    await interaction.followup.send(embed=embeds[current], ephemeral=True, view=PageView())
        ...
        if interaction.user.id == message.user.id:
            ...
            try:
                ...
                async for p in Prism.filter("guild_id = $1 AND user_id = $2", message.guild.id, person_id.id):
                    ...
                    await p.save()
                await interaction.edit_original_response(
                    content=f"Done! rip {person_id.mention}. f's in chat.\njoin our discord to rollback: <https://discord.gg/GCYRuTWnpr>", view=None
                )
            except Exception:
                await interaction.edit_original_response(
    ...
    async def count(interaction: discord.Interaction):
        ...
        if interaction.user.id == message.user.id:
            ...
            if counter == 0:
                ...
                try:
                    await interaction.edit_original_response(
                        content="Done. If you want to roll this back, please contact us in our discord: <https://discord.gg/GCYRuTWnpr>.",
                        view=None,
                    )
                except Exception:
                    await interaction.followup.send("Done. If you want to roll this back, please contact us in our discord: <https://discord.gg/GCYRuTWnpr>.")
            else:
...
async def setup(bot2):
    ...
    # === FIX START: Correctly define and attach the vote handler ===

    # Define the vote handler function without 'self'
    async def handle_vote_callback(user_id: str):
        try:
            # 1. Database Logic
            # User must be imported from your database models
            user = await User.get_or_create(user_id=int(user_id))
            now = int(time.time())

            if user.vote_time_topgg + 43100 > now:
                print(f"⏳ Duplicate vote ignored from {user_id}")
                return

            user.vote_time_topgg = now
            user.vote_streak += 1
            await user.save()

            print(f"✅ Vote received and DB updated from {user_id}")

            # 2. Discord DM Logic
            # Use the global 'bot' variable to get the user
            voter = bot.get_user(int(user_id))
            if voter:
                try:
                    await voter.send("Thanks for voting for Cat! 🐾")
                except Exception:
                    # Failed to DM, maybe user has DMs disabled
                    pass

        except Exception as e:
            # Corrected indentation for print statement
            print(f"❌ Error handling vote from {user_id}: {e}")

    # Attach the handler function to the bot object
    # The webhook server will now call bot.handle_vote(user_id)
    bot.handle_vote = handle_vote_callback

    # Start webhook server in the background, which will now find the attached handle_vote method
    asyncio.create_task(webhook_server.start_webhook_server(bot))
